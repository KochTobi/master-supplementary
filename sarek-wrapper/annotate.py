...
def parse(argv):
    parser = argparse.ArgumentParser(description="Wrapper for the sarek annotate.nf pipeline.")
    ...
    required_args.add_argument('-t', '--tools', type=str, nargs=None,
                               help='list of tools to be used for annotation e.g.: \'snpeff,VEP\'',
                               required=True)
    ...
    optional_args.add_argument('-l', '--logfile', metavar='LOGFILE', dest='logfile', type=str,
                               default='.annotate.nextflow.log',
                               nargs=None,
                               help='name for the nextflow logfile in the reports directory, default: .nextflow.log')
    optional_args.add_argument('--annotateTools', type=str, nargs=None,
                               help='list of tools to be used by sarek e.g.: \'HaplotypeCaller,Strelka\'')
    ...
    optional_args.add_argument('--script_location', metavar='SCRIPT', type=str, default='annotate.nf', nargs=None,
                               help='path to the annotate.nf script')
...
def annotate_sample(aws_queue, profile, annotate_tools, tools, out_dir, work, script, genome, genome_base, resume,
                    nextflow_args,
                    local_report_dir, logfile, dry_run=False):
    nextflow_log = logfile
    command = "nextflow -log %s run %s -profile %s --awsqueue %s %s --tools %s --genome_base %s --genome %s --outDir %s -w %s %s --localReportDir %s --verbose -with-report -with-trace -with-timeline -with-dag -ansi %s" % (
        nextflow_log, script, profile, aws_queue, annotate_tools, tools, genome_base, genome, out_dir, work,
        nextflow_args,
    ...
    if not dry_run:
        try:
            ...
            raise RuntimeError('Sarek annotate.nf script unsuccessful.') from err
    return
...
def main(argv):
    ...
    try:
        annotate_sample(aws_queue=aws_queue, profile=profile, annotate_tools=annotate_tools, tools=tools,
                        out_dir=out_dir,
                        work=work,
